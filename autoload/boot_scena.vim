...
function! boot_scena#Activate(...)
  ...
  """ Assist Hashira
  nnoremap <buffer><silent> <C-v><C-k> i<C-r>=scena#Assist("g:hashira_dic",0,0)<CR>
  inoremap <buffer><silent> <C-v><C-k> <C-r>=scena#Assist("g:hashira_dic",0,0)<CR>

  """ Assist Scene
  nnoremap <buffer><silent> <C-v><C-j> i<C-r>=scena#SceneComp()<CR>
  inoremap <buffer><silent> <C-v><C-j> <C-r>=scena#SceneComp()<CR>
  ...
  """ Serifu tag
  nnoremap <buffer><silent> <C-v><C-i> i<C-r>=scena#Serifutag(g:serifu_dic)<CR>
  inoremap <buffer><silent> <C-v><C-i> <C-r>=scena#Serifutag(g:serifu_dic)<CR>
  ...
  """ Assist Symbol
  nnoremap <buffer><silent> <C-v><C-u> i<C-r>=scena#Assist("g:symbol_dic",0,0)<CR>
  inoremap <buffer><silent> <C-v><C-u> <C-r>=scena#Assist("g:symbol_dic",0,0)<CR>

  """ gomibox
  vnoremap <buffer><silent> <C-g> x:call scena#Gomibox()<CR>

  """ info
  nnoremap <buffer><silent> <F1> :call <SID>Info()<CR>

  """ SetForm
  nnoremap <buffer><silent> <F8> :call <SID>SetForm(0)<CR>

  """ Reload NameDic
  nnoremap <buffer><silent> <F9> :call <SID>NameDic()<CR>
  inoremap <buffer><silent> <F9> <Esc>:call <SID>NameDic()<CR>

  """ JpCountPages
  nnoremap <buffer><silent> <F10> :call scena#JCP()<CR>
  inoremap <buffer><silent> <F10> <Esc>:call scena#JCP()<CR>

  """ Journal
  nnoremap <buffer><silent> <F11> :call scena#Journal()<CR>
  inoremap <buffer><silent> <F11> <Esc>:call scena#Journal()<CR>

  """ Memo
  nnoremap <buffer><silent> <F12> :call scena#LineFeed('｛MEMO;｝',1)<CR>$i
  inoremap <buffer><silent> <F12> <Esc>:call scena#LineFeed('｛MEMO;｝',1)<CR>$i

  """ Unite Search
  nnoremap <buffer><silent> /  :<C-u>Unite -buffer-name=search
    \ -horizontal
    \ line:forward -start-insert -no-quit<CR>

  """ Show Unite Outline
  nnoremap <buffer><silent> <C-q> :call <SID>StartUnite()<CR>
  inoremap <buffer><silent> <C-q> <Esc>:call <SID>StartUnite()<CR>i
